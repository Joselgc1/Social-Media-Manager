...
async def test_store_migration_runner_executes_schema_and_closes_connection(monkeypatch):
    ...
    instagram_story_context_migration_sql = runner.INSTAGRAM_STORY_CONTEXT_MIGRATION_PATH.read_text(
        encoding="utf-8"
    )
    ...
    media_cache_migration_sql = runner.KOMMO_MEDIA_CACHE_MIGRATION_PATH.read_text(encoding="utf-8")
    media_content_hash_migration_sql = runner.KOMMO_MEDIA_CONTENT_HASH_MIGRATION_PATH.read_text(
        encoding="utf-8"
    )
    inbound_voice_migration_sql = runner.KOMMO_INBOUND_VOICE_MIGRATION_PATH.read_text(encoding="utf-8")
    inbound_attachments_migration_sql = runner.KOMMO_INBOUND_ATTACHMENTS_MIGRATION_PATH.read_text(
        encoding="utf-8"
    )
    conversation_scope_migration_sql = (
        runner.CONVERSATION_INTERACTION_SCOPE_MIGRATION_PATH.read_text(encoding="utf-8")
    )
    waiting_delivery_migration_sql = runner.KOMMO_WAITING_FOR_DELIVERY_MIGRATION_PATH.read_text(
        encoding="utf-8"
    )
    ...
    assert connection.execute.await_args_list[10].args == (media_cache_migration_sql,)
    assert connection.execute.await_args_list[11].args == (media_content_hash_migration_sql,)
    ...
    assert connection.execute.await_args_list[15].args == (waiting_delivery_migration_sql,)
