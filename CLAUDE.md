...
**Database:** Supabase PostgreSQL. Fresh installs run `store/migrations/001_schema.sql` once; existing unversioned stores run `store/migrations/002_existing_database_upgrade.sql` instead. Never rerun `001` as an upgrade. `schema_migrations` records the version and startup rejects incompatible schemas. Tables include customers, conversations, orders, broadcasts, settings, usage_log, ai_run_logs, daily_analytics, product_analytics, conversation_sessions, customer_channel_mappings, meta_inbound_jobs, meta_inbound_receipts, kommo_message_jobs, and kommo_message_receipts.
