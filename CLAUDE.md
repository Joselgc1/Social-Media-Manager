...
For local debug, either `OPENAI_API_KEY` or `ANTHROPIC_API_KEY` plus `DATABASE_URL`, `GOOGLE_SHEETS_CREDENTIALS_B64`, and `PRODUCT_SHEET_ID` are the core minimum. Meta, Kommo, and Telegram fields may stay empty when `DEBUG=true` unless you are testing that specific channel backend. In production (`DEBUG=false`), startup validation requires a non-placeholder `ADMIN_PASSWORD` of at least 12 characters, at least one real LLM key, and the complete non-placeholder credential set for the selected `CHANNEL_BACKEND`: Meta mode requires the full WhatsApp config (`META_APP_SECRET`, `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_VERIFY_TOKEN`); Kommo mode requires the Kommo private integration, private-message Salesbot, webhook secret, and AI Mode field/enum vars. Instagram remains optional and all-or-nothing. Telegram is optional but requires `TELEGRAM_BOT_TOKEN`, `TELEGRAM_ADMIN_CHAT_ID`, and `TELEGRAM_WEBHOOK_SECRET` together.
...
**Database:** Supabase PostgreSQL. Fresh installs run `store/migrations/001_schema.sql` once; existing unversioned stores run `store/migrations/002_existing_database_upgrade.sql`, then all later numbered store migrations in order. Never rerun `001` as an upgrade. `schema_migrations` records the version and startup rejects incompatible schemas. Tables include customers, conversations, orders, broadcasts, settings, usage_log, ai_run_logs, daily_analytics, product_analytics, conversation_sessions, customer_channel_mappings, meta_inbound_jobs, meta_inbound_receipts, kommo_message_jobs, and kommo_message_receipts.
...
**Database:** Separate Supabase project. Fresh databases run `master/migrations/001_master_schema.sql`; existing unversioned databases run `master/migrations/002_existing_database_upgrade.sql`. Tables: `stores`, `store_credentials`, `master_audit_log`, `exchange_rates`, `schema_migrations`.
...
- **Migrations are manual and versioned:** Run `001_schema.sql` only for a fresh store database. Run `002_existing_database_upgrade.sql` for an existing unversioned database, then apply future numbered migrations in order. Startup requires the exact supported `schema_migrations` version.
...
- **Master migrations are manual too:** Run `001_master_schema.sql` only for a fresh master database or `002_existing_database_upgrade.sql` for an existing unversioned database. Startup checks `schema_migrations`.
