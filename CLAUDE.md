...
- Store-dashboard and Telegram per-customer pauses in Kommo mode must sync Kommo first: set lead `AI Mode` to `Paused`, re-read and verify the enum, then persist the local manual escalation. A failed or unconfirmed provider update must not change local state. Global AI pause remains a Store setting and does not update individual Kommo leads.
