...
- Store-only `accepted_exchange_rate`, `order_discount_percent`, and `order_discount_threshold_usd` settings also live in the store `settings` table and are edited only from the store dashboard. Eva uses the exchange rate to answer rate questions like `¿a qué tasa recibes?`; the prompt and order backend use the discount settings for automatic subtotal-based discounts.
...
- Kommo jobs carry `interaction_type`: `private_message` for WhatsApp/Instagram DMs and `instagram_comment` for public comment replies. The parser defaults to `private_message` and does not infer comments from `origin=instagram`; map native comment fields only after a real Kommo comment payload is captured and verified.
...
- **Unsupported or gated in Kommo mode:** native rich-media delivery through Kommo, automatic human takeover from outgoing/native phone replies, Kommo broadcasts from this backend, and verified production Salesbot/comment delivery without testing in a real Kommo account. Native public Instagram comment classification remains gated until a real Kommo comment webhook payload is captured and mapped.
