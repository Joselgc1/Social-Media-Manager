...
4. Run the service migration runner for the normal deployment path. The Store runner applies its sequence through version 16; the Master runner applies its baseline. Use `002_consolidated_upgrade.sql` only for a documented pre-consolidation recovery case, then rerun the normal runner.
