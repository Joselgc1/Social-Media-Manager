...
KOMMO_WAITING_FOR_DELIVERY_MIGRATION_PATH = (
    Path(__file__).resolve().parents[1] / "migrations" / "016_kommo_waiting_for_delivery.sql"
)
...
async def run_migration(database_url: str | None = None) -> None:
    ...
    kommo_waiting_for_delivery_migration_sql = (
        KOMMO_WAITING_FOR_DELIVERY_MIGRATION_PATH.read_text(encoding="utf-8")
    )
    ...
    try:
        ...
        try:
            ...
            await connection.execute(kommo_waiting_for_delivery_migration_sql)
        finally:
