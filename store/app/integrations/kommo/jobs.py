...
async def persist_salesbot_callback(data: SalesbotWidgetData, return_url: str) -> dict:
    job = await _find_waiting_job_for_callback(data)
    if not job:
        return {"status": "ignored", "reason": "no_waiting_job"}
    if job["status"] in {"sent", "discarded", "failed"}:
        return {"status": "duplicate", "job_id": str(job["id"])}
    ...
    await db.execute(
        ...
        """,
        {"return_url": return_url, "id": job["id"]},
    )
...
async def diagnostics_summary() -> dict:
    ...
    return {
        "pending_job_count": counts.get("pending", 0) + counts.get("waiting_for_salesbot", 0) + counts.get("ready", 0),
            ...
            WHERE candidate.status = 'pending'
              ...
              AND NOT EXISTS (
                  ...
                  WHERE active.correlation_id = candidate.correlation_id
                    AND active.status IN ('waiting_for_salesbot', 'ready', 'processing')
...
async def _process_ready_job(job: dict) -> None:
    ...
    try:
        ...
        if not before.allowed:
            await _store_user_message_if_suppressed(customer, job)
            await _mark_job(job["id"], "discarded", before.reason)
            return

        sender_id = _local_sender_id(job)
        media_url = job.get("media_url") if job.get("channel") == "instagram" else None
        ...
        if not result.get("escalated"):
            ...
            if not after.allowed:
                await _mark_job(job["id"], "discarded", after.reason)
                return

        mapped = map_ai_response_to_salesbot(result)
        if mapped.discarded:
            await _mark_job(job["id"], "discarded", mapped.reason or "empty_response")
            return
        await client.continue_salesbot(job["return_url"], mapped.execute_handlers)
        await _mark_job(job["id"], "sent", None)
...
async def _find_waiting_job_for_callback(data: SalesbotWidgetData):
    clauses = ["status = 'waiting_for_salesbot'"]
    values = {}
    if data.lead_id:
...
async def _mark_job(job_id: str, status: str, error: str | None) -> None:
    await db.execute(
        ...
        SET status = :status,
            ...
            processing_started_at = NULL,
            completed_at = CASE WHEN :status IN ('sent', 'discarded', 'failed') THEN NOW() ELSE completed_at END,
            updated_at = NOW()
        WHERE id = :id
        """,
